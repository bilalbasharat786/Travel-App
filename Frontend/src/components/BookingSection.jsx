...
// Assets se images aur icons import karein
import DestinationIcon from '../assets/icon-1.png'; // Yellow icon
import PaymentIcon from '../assets/icon-2.png';     // Orange icon
import AirportIcon from '../assets/icon-3.png';     // Blue icon
import GreeceTripImg from '../assets/greece.jpg';   // Main trip image
import RomeMiniImg from '../assets/rome-small.png'; // Small circular image
import HeartIcon from '../assets/heart.png';       // Blue heart icon
import LeafIcon from '../assets/leaf.png'; // Decorative leaf image
  ...
  return (
    <section className="py-20 px-6 max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
      ...
      <div className="w-full lg:w-1/2">
        <p className="text-[#5E6282] font-semibold text-lg mb-2">Easy and Fast</p>
          ...
          {steps.map((step) => (
            <div key={step.id} className="flex items-start gap-5">
              <div className={`${step.bgColor} rounded-xl`}>
                <img src={step.icon} alt="icon" className="w-10 h-10 rounded-xl " />
              </div>
              <div>
                <h4 className="text-[#5E6282] font-bold text-base">{step.title}</h4>
      ...
      <div className="w-full lg:w-1/2 relative flex justify-center items-center mt-20 lg:mt-0">
        ...
        <div className="bg-white rounded-[26px] shadow-2xl p-6 w-full max-w-[370px] relative z-10">
          ...
          <p className="text-[#84829A] text-sm font-medium mb-4">14-29 June | by Robbin joseph</p>
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <div className='h-6 w-6  ' ><img src={BuildingIcon} alt="building" /></div>
              <span className="text-[#84829A] text-sm font-medium">24 people going</span>
            </div>
            <img src={HeartIcon} alt="wishlist" className="w-5 h-5 cursor-pointer" />
          </div>
        </div>

        {/* Floating Mini Card (Ongoing Trip) */}
        <div className="absolute bottom-10 -right-4 md:-right-10 bg-white shadow-2xl rounded-[20px] p-4 flex gap-4 w-[240px] z-20 animate-bounce-slow">
          ...
          <div className="flex-1">
            <p className="text-[#84829A] text-[10px] font-medium uppercase tracking-tighter">Ongoing</p>
            <h4 className="text-black text-sm font-bold mb-2">Trip to rome</h4>
            <p className="text-sm font-medium mb-1"><span className="text-[#8A79DF]">40%</span> completed</p>
